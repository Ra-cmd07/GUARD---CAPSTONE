  ...
  try {
    ...
    if (role === 'parent' && profileId) {
      ...
      const [studentLinks] = await pool.execute(
        `SELECT s.id, s.lrn, s.name, s.gender, s.grade, s.section_id, sec.name AS section_name,
...
export async function createStudent(req: AuthRequest, res: Response): Promise<void> {
  ...
  try {
    ...
    const [result] = await conn.execute(
      `INSERT INTO students (lrn, name, gender, grade, section_id, mac_address, rfid_uid, preferred_method, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [lrn, name,
...
export async function updateStudent(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    await pool.execute(
      `UPDATE students SET name=?, gender=?, grade=?, section_id=?, mac_address=?, rfid_uid=?, preferred_method=?, updated_by=?
       WHERE id=?`,
      [name,
