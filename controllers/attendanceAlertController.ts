...
export async function checkAttendanceThresholds(): Promise<void> {
    ...
    for (const s of students) {
      ...
      const [existing]: any = await pool.query(
        `SELECT id FROM attendance_alerts
         WHERE student_id = ? AND month_year = ? AND alert_type = 'low_attendance'`,
        [s.student_id, monthYear]
      );
      ...
      await pool.query(
        `INSERT INTO attendance_alerts
         ...
         ON DUPLICATE KEY UPDATE attendance_rate = VALUES(attendance_rate)`,
        [s.student_id, s.rate_pct, THRESHOLD, monthYear]
      );

      // ── Notify teacher ─────────────────────────────────────────────
      const [tRows]: any = await pool.query(
        `SELECT t.user_id FROM teachers t
         WHERE LOWER(t.section) = LOWER(?) LIMIT 1`,
        [s.section]
      );
      if (tRows.length) {
        await createNotification(
          tRows[0].user_id,
          `⚠️ Low Attendance: ${s.student_name}`,
          `${s.student_name}'s attendance rate is ${s.rate_pct}% this month (below ${THRESHOLD}% threshold).`,
          'warning',
          '/teacher'
        );
        await pool.query(
          `UPDATE attendance_alerts SET notified_teacher = 1
      ...
      const [parentRows]: any = await pool.query(
        `SELECT p.user_id, p.contact FROM parent_student ps
      ...
      for (const parent of parentRows) {
        await createNotification(
          parent.user_id,
          `⚠️ Attendance Alert: ${s.student_name}`,
          `${s.student_name} has an attendance rate of ${s.rate_pct}% this month. Please contact the school.`,
          'warning',
          '/parent'
        );
        if (parent.contact) {
          ...
          await pool.query(
            ...
            [parent.contact, sms.slice(0, 160), s.student_id]
          );
        }
      }
      if (parentRows.length) {
        await pool.query(
          `UPDATE attendance_alerts SET notified_parent = 1
           WHERE student_id = ? AND month_year = ?`,
          [s.student_id, monthYear]
        );
      }
      ...
      for (const admin of adminRows) {
        await createNotification(
          admin.id,
          `⚠️ At-Risk Student: ${s.student_name}`,
          `${s.student_name} (${s.section}) has ${s.rate_pct}% attendance this month — below the ${THRESHOLD}% threshold.`,
          'warning',
          '/admin'
        );
      }
      if (adminRows.length) {
        await pool.query(
          `UPDATE attendance_alerts SET notified_admin = 1
           WHERE student_id = ? AND month_year = ?`,
          [s.student_id, monthYear]
        );
      }
...
export async function getAdminAlerts(req: AuthRequest, res: Response) {
  try {
    const monthYear = (req.query.month as string) || format(new Date(), 'yyyy-MM');
...
export async function getTeacherAlerts(req: AuthRequest, res: Response) {
  try {
...
export async function getParentAlerts(req: AuthRequest, res: Response) {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });
...
export async function dismissAlert(req: AuthRequest, res: Response) {
  try {
    ...
    try {
      if (role === 'admin') {
        await pool.query(
          `UPDATE attendance_alerts SET dismissed_by_admin = NOW() WHERE id = ?`,
          [id]
        );
      } else if (role === 'parent') {
        await pool.query(
          `UPDATE attendance_alerts SET dismissed_by_parent = NOW() WHERE id = ?`,
          [id]
        );
      } else {
...
export async function manualRunCheck(req: AuthRequest, res: Response) {
