...
async function fetchRfidData(uid?: string) {
  ...
  const queryParams: any[] = params.length > 0 ? params : [];
...
export async function createRfidLog(req: Request, res: Response): Promise<void> {
  try {
    ...
    try {
      const [rows] = await pool.execute(`SELECT name FROM rfid_tags WHERE UPPER(uid) = UPPER(?)`, [uid]) as any;
    ...
    await pool.execute(
      `INSERT INTO RFID_LOGS (uid, USTP_CDO, timestamp, student_name)
       VALUES (?, ?, ?, ?)`,
      [uid.toUpperCase(), location, timestamp, studentName]
    );
...
export async function getRfidLogs(req: Request, res: Response): Promise<void> {
  try {
...
export async function getRfidDashboard(req: Request, res: Response): Promise<void> {
  try {
    const uidFilter = typeof req.query.uid === 'string'
      ? req.query.uid.trim()
      : undefined;
