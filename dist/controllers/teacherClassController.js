...
exports.getTeacherClassDetail = getTeacherClassDetail;
exports.updateTeacherClass = updateTeacherClass;
...
async function getTeacherClasses(req, res) {
    try {
        ...
        const [classes] = await db_1.default.query(`SELECT 
        tc.id,
        tc.teacher_id,
        tc.section_id,
        tc.subject,
...
async function getTodayClasses(req, res) {
    try {
        ...
        const dayOfWeek = (0, date_fns_1.format)(new Date(), 'EEEE'); // e.g., "Monday"
        const currentTime = (0, date_fns_1.format)(new Date(), 'HH:mm:ss');
        // Get all classes for today
        const [todayClasses] = await db_1.default.query(`SELECT 
        tc.id,
        tc.teacher_id,
        tc.section_id,
        tc.subject,
      ...
      WHERE tc.teacher_id = ? 
        ...
        AND tc.is_active = 1
      GROUP BY tc.id
      ORDER BY tc.time_start`, [currentTime, currentTime, currentTime, dbTeacherId, dayOfWeek]);
        // Separate into current, upcoming, and completed
        const current = todayClasses.filter((c) => c.class_status === 'current');
        const upcoming = todayClasses.filter((c) => c.class_status === 'upcoming');
        const completed = todayClasses.filter((c) => c.class_status === 'completed');
        res.json({
            success: true,
...
async function createTeacherClass(req, res) {
    try {
        ...
        const [teacher] = await db_1.default.query(`SELECT id FROM teachers WHERE id = ?`, [teacher_id]);
        if (!teacher || teacher.length === 0) {
            return res.status(404).json({ error: 'Teacher not found' });
        }
        const dbTeacherId = teacher[0].id;
        // Verify section exists
        const [section] = await db_1.default.query(`SELECT id, name FROM sections WHERE id = ?`, [section_id]);
        if (!section || section.length === 0) {
            return res.status(404).json({ error: 'Section not found' });
        }
        // Check for duplicate schedule (same teacher, section, time, day)
        const [duplicate] = await db_1.default.query(`SELECT id FROM teacher_classes 
       WHERE teacher_id = ? 
         AND section_id = ? 
         AND time_start = ? 
         AND day_of_week = ?
         AND is_active = 1`, [dbTeacherId, section_id, time_start, day_of_week]);
        ...
        // Insert new class
        const [result] = await db_1.default.query(`INSERT INTO teacher_classes 
       (teacher_id, section_id, subject, time_start, time_end, day_of_week, room_number, capacity, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [dbTeacherId, section_id, subject, time_start, time_end, day_of_week, room_number, capacity, adminUserId]);
        res.status(201).json({
            success: true,
...
async function getClassAttendance(req, res) {
    try {
        ...
        const [classRecord] = await db_1.default.query(`SELECT tc.*, s.name as section_name 
       FROM teacher_classes tc
       LEFT JOIN sections s ON tc.section_id = s.id
       WHERE tc.id = ? AND tc.teacher_id = ?`, [classId, dbTeacherId]);
       ...
       ORDER BY a.timestamp DESC`, [classId, date]);
 ...
 */
async function getTeacherClassDetail(req, res) {
    try {
        const teacherId = req.user?.id;
        const { classId } = req.params;
        if (!teacherId) {
        ...
        const dbTeacherId = teacher[0].id;
        // Get class details
        const [classRecord] = await db_1.default.query(`SELECT 
        tc.*,
        s.name as section_name,
        s.grade,
        s.section_code,
       ...
       GROUP BY tc.id`, [classId, dbTeacherId]);
        if (!classRecord || classRecord.length === 0) {
            return res.status(404).json({ error: 'Class not found' });
        }
        res.json({
...
async function updateTeacherClass(req, res) {
    try {
        ...
        const [classRecord] = await db_1.default.query(`SELECT id, teacher_id FROM teacher_classes WHERE id = ?`, [classId]);
        if (!classRecord || classRecord.length === 0) {
            return res.status(404).json({ error: 'Class not found' });
        }
        // Update class
        ...
        await db_1.default.query(`UPDATE teacher_classes SET ${updateFields.join(', ')} WHERE id = ?`, updateValues);
        res.json({
            success: true,
...
async function deleteTeacherClass(req, res) {
    try {
        ...
        // Verify the class exists
        const [classRecord] = await db_1.default.query(`SELECT id FROM teacher_classes WHERE id = ?`, [classId]);
        if (!classRecord || classRecord.length === 0) {
            return res.status(404).json({ error: 'Class not found' });
        }
        // Soft delete (mark as inactive)
        await db_1.default.query(`UPDATE teacher_classes SET is_active = 0, updated_by = ? WHERE id = ?`, [adminUserId, classId]);
        res.json({
            success: true,
