...
exports.getAssignmentsGrouped = getAssignmentsGrouped;
exports.getAssignmentsMetadata = getAssignmentsMetadata;
exports.createAssignment = createAssignment;
exports.updateAssignment = updateAssignment;
exports.deleteAssignment = deleteAssignment;
exports.updateSectionAdviser = updateSectionAdviser;
exports.updateSectionStudents = updateSectionStudents;
const db_1 = __importDefault(require("../lib/db"));
// Cache for runtime schema checks to avoid querying information_schema on every request
let _subjectTeacherColumnCache = {};
let _sessionColumnCache = {};
async function hasSubjectTeacherColumn() {
    const now = Date.now();
    if (_subjectTeacherColumnCache.ts && now - (_subjectTeacherColumnCache.ts || 0) < 60000) {
        return !!_subjectTeacherColumnCache.has;
    }
    try {
        const [rows] = await db_1.default.execute(`SELECT COUNT(*) AS cnt FROM information_schema.COLUMNS
...
async function hasSessionColumn() {
    const now = Date.now();
    if (_sessionColumnCache.ts && now - (_sessionColumnCache.ts || 0) < 60000) {
...
async function getAssignmentsGrouped(_req, res) {
    try {
        const includeSession = await hasSessionColumn();
        const sessionCol = includeSession ? ', a.session' : ", 'AM' AS session";
        const [rows] = await db_1.default.execute(`SELECT a.id, a.year_level, a.strand, a.track, a.section, a.subject
...
async function createAssignment(req, res) {
    ...
    try {
        ...
        if (includeSubjectTeacher && includeSession) {
            [result] = await conn.execute(`INSERT INTO assignments
           (year_level, strand, track, section, subject, teacher_id, subject_teacher_id, session, created_by)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [year_level, strand || null, track || null, section, subject,
...
async function updateAssignment(req, res) {
    ...
    try {
        ...
        if (includeSubjectTeacher && includeSession) {
         ...
         WHERE id = ?`, [year_level, strand || null, track || null, section, subject,
...
async function updateSectionAdviser(req, res) {
    try {
        ...
        await db_1.default.execute(`UPDATE assignments
...
async function updateSectionStudents(req, res) {
    const conn = await db_1.default.getConnection();
    try {
        await conn.beginTransaction();
        ...
        const [asgRows] = await conn.execute(`SELECT id FROM assignments
       WHERE year_level = ?
         AND (strand IS NULL AND ? IS NULL OR strand = ?)
         AND (track IS NULL AND ? IS NULL OR track = ?)
         AND section = ?`, [
        ...
        const assignmentIds = asgRows.map((r) => r.id);
        ...
        for (const asgId of assignmentIds) {
            await conn.execute('DELETE FROM assignment_students WHERE assignment_id = ?', [asgId]);
        }
        // Re-insert students for all assignment rows in this section
        if (Array.isArray(student_ids) && student_ids.length > 0) {
            for (const asgId of assignmentIds) {
                const values = student_ids.map((studentId) => [asgId, studentId, req.user.id]);
                await conn.query(`INSERT IGNORE INTO assignment_students (assignment_id, student_id, created_by)
           VALUES ?`, [values]);
            }
        }
