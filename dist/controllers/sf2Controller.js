...
exports.sf2Upload = void 0;
exports.generateSF2 = generateSF2;
exports.getSF2Preview = getSF2Preview;
exports.getCoordinationStatus = getCoordinationStatus;
exports.inspectTemplate = inspectTemplate;
exports.getSF2History = getSF2History;
...
exports.sf2Upload = (0, multer_1.default)({
    dest: path_1.default.join(__dirname, '..', '..', 'uploads', 'sf2_tmp'),
...
/* ─── Output directory ────────────────────────────────────────────────────── */
const UPLOAD_DIR = path_1.default.join(__dirname, '..', '..', 'uploads', 'sf2');
...
function buildDrawingXml(amCells, pmCells) {
    let shapeId = 100;
    const shapes = [];
    const makeShape = (pos, isAM, id) => {
    ...
    for (const pos of amCells)
        shapes.push(makeShape(pos, true, shapeId++));
    for (const pos of pmCells)
        shapes.push(makeShape(pos, false, shapeId++));
...
async function patchDrawings(filePath, amCells, pmCells) {
    ...
    if (!sheetKey)
        throw new Error('Could not find worksheet in xlsx');
    const sheetXml = await zip.file(sheetKey).async('string');
    const sheetName = path_1.default.basename(sheetKey);
    ...
    if (!relsXml.includes('drawing')) {
        relsXml = relsXml.replace('</Relationships>', `<Relationship Id="${drawingRelId}" ` +
            `Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing" ` +
            `Target="../drawings/drawing1.xml"/>\n</Relationships>`);
    }
    let newSheetXml = sheetXml;
    if (!newSheetXml.includes('<drawing')) {
        newSheetXml = newSheetXml.replace('</worksheet>', `<drawing r:id="${drawingRelId}"/></worksheet>`);
        if (!newSheetXml.includes('xmlns:r=')) {
            newSheetXml = newSheetXml.replace('<worksheet ', '<worksheet xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" ');
        }
    }
    const drawingXml = buildDrawingXml(amCells, pmCells);
    let ctXml = await zip.file('[Content_Types].xml').async('string');
    if (!ctXml.includes('drawing1.xml')) {
        ctXml = ctXml.replace('</Types>', `<Override PartName="/xl/drawings/drawing1.xml" ` +
...
async function getFormattedName(studentName, lrn) {
    try {
        if (lrn) {
            const [rows] = await db_1.default.execute(`SELECT last_name, first_name, middle_name, name
         FROM students WHERE lrn = ? LIMIT 1`, [lrn]);
            const s = rows[0];
...
═══════════════════════════════════════════════════════════════════════════ */
async function buildStudentMeta(month, year, section) {
    ...
    const [rows] = await db_1.default.execute(`SELECT
     ...
     ORDER BY a.student_name, a.date, a.session, is_subject ASC, a.id ASC`, params);
    const meta = {};
    for (const r of rows) {
        ...
        if (!meta[name].gender && gender)
            meta[name].gender = gender;
        if (!meta[name].days[day])
            meta[name].days[day] = { am: false, pm: false };
    ...
    if (section?.trim()) {
        try {
            const [assignedStudents] = await db_1.default.execute(`SELECT DISTINCT s.id, s.name, s.lrn, s.gender,
         ...
         ORDER BY s.name`, [section.trim()]);
            for (const s of assignedStudents) {
                ...
                if (!meta[studentName]) {
                    ...
                    if (!meta[studentName].gender && gender)
                        meta[studentName].gender = gender;
                    if (meta[studentName].displayName === studentName && displayName !== studentName) {
                        meta[studentName].displayName = displayName;
                    }
                    if (!meta[studentName].lrn && lrn)
                        meta[studentName].lrn = lrn;
                }
            }
        }
        catch (err) {
...
═══════════════════════════════════════════════════════════════════════════ */
async function generateSF2(req, res) {
    const file = req.file;
    if (!file)
        return res.status(400).json({ error: 'Upload template file' });
    const month = Number(req.body.month);
    const year = Number(req.body.year);
    const section = (req.body.section || '').trim() || req.teacher?.section?.trim() || '';
    ...
    let schoolSettings = {};
    try {
        const [rows] = await db_1.default.execute('SELECT school_id, school_name, school_year, school_head_name FROM school_settings WHERE id = 1');
        if (rows.length > 0)
            schoolSettings = rows[0];
    }
    ...
    try {
        if (schoolSettings.school_id)
            ws.getCell(6, 3).value = schoolSettings.school_id;
        if (schoolSettings.school_year)
            ws.getCell(6, 11).value = schoolSettings.school_year;
        ws.getCell(6, 24).value = `${monthName} ${year}`;
        if (schoolSettings.school_name)
            ws.getCell(8, 3).value = schoolSettings.school_name;
        // Grade Level — fetch from assignments table using section name
        try {
            const [yrRows] = await db_1.default.query(`SELECT year_level FROM assignments
         WHERE LOWER(TRIM(section)) = LOWER(TRIM(?))
           AND year_level IS NOT NULL AND year_level != ''
         LIMIT 1`, [section]);
            const yearLevel = yrRows[0]?.year_level || '';
            ws.getCell(8, 24).value = yearLevel || null;
        }
        catch {
            ws.getCell(8, 24).value = null;
        }
        if (section)
            ws.getCell(8, 29).value = section;
    }
    catch (headerErr) {
    ...
    if (maleHeaderRow > 0) {
        for (let c2 = 1; c2 <= 4; c2++) {
            ...
            if (v) {
                console.log(`[SF2] maleHeaderRow ${maleHeaderRow} col ${c2}: "${v}"`);
                break;
            }
        }
    }
    if (femaleHeaderRow > 0) {
        for (let c2 = 1; c2 <= 4; c2++) {
            ...
            if (v) {
                console.log(`[SF2] femaleHeaderRow ${femaleHeaderRow} col ${c2}: "${v}"`);
                break;
            }
        }
    }
    ...
    function fillSection(list, startRow, endRow) {
        list.forEach((name, i) => {
    ...
    for (const [, meta] of Object.entries(studentMeta)) {
        ...
        for (const [dayStr, flags] of Object.entries(meta.days)) {
            ...
            if (flags.am || flags.pm) {
                if (isM)
                    maleDayCount[day] = (maleDayCount[day] || 0) + 1;
                if (isF)
                    femaleDayCount[day] = (femaleDayCount[day] || 0) + 1;
            }
        }
    }
    console.log(`[SF2] Day counts — Male: ${JSON.stringify(maleDayCount)}, Female: ${JSON.stringify(femaleDayCount)}`);
    const writeTotal = (headerRow, dayCounts, label) => {
        if (headerRow < 1) {
            console.log(`[SF2] SKIP ${label} — row not detected`);
            return;
        }
        let written = 0;
    ...
    for (let r = 1; r <= 70; r++) {
        for (let c2 = 1; c2 <= 10; c2++) {
            ...
            if (detectedFemale < 0 && cellStr.includes('FEMALE')) {
                detectedFemale = r;
                console.log(`[SF2] FEMALE label row ${r} col ${c2}: "${rawVal}"`);
            }
            if (detectedMale < 0 && !cellStr.includes('FEMALE') && cellStr.includes('MALE')) {
                detectedMale = r;
                console.log(`[SF2] MALE label row ${r} col ${c2}: "${rawVal}"`);
            }
            if (detectedCombined < 0 && cellStr.includes('COMBINED')) {
                detectedCombined = r;
                console.log(`[SF2] COMBINED label row ${r} col ${c2}: "${rawVal}"`);
            }
        }
    }
    ...
    const outputPath = path_1.default.join(UPLOAD_DIR, `SF2_${Date.now()}.xlsx`);
    ...
    const fileStream = fs_1.default.createReadStream(outputPath);
    fileStream.pipe(res);
    fileStream.on('end', () => fs_1.default.unlink(outputPath, () => { }));
    fileStream.on('error', () => { if (!res.headersSent)
        res.status(500).end(); });
}
/* ═══════════════════════════════════════════════════════════════════════════
   CONTROLLER: GET /api/sf2/preview
═══════════════════════════════════════════════════════════════════════════ */
async function getSF2Preview(req, res) {
    try {
...
═══════════════════════════════════════════════════════════════════════════ */
async function getCoordinationStatus(req, res) {
    try {
        const { month, year, section } = req.query;
        ...
        const [subjectRows] = await db_1.default.execute(`SELECT
         a.subject,
       ...
       ORDER BY att.subject`, [section, startDate, endDate]);
        const totalDays = daysInMonth(m, y);
        ...
        const [advisoryRows] = await db_1.default.execute(`SELECT COUNT(DISTINCT date) AS days_recorded, COUNT(*) AS total_records
       FROM attendance
       WHERE section = ? AND date BETWEEN ? AND ?
         AND (subject IS NULL OR subject = '')`, [section, startDate, endDate]);
        res.json({
            section,
            month: m,
            year: y,
...
═══════════════════════════════════════════════════════════════════════════ */
async function inspectTemplate(req, res) {
    const file = req.file;
    if (!file)
        return res.status(400).json({ error: 'Upload template file' });
    try {
...
async function getSF2History(_req, res) {
    try {
        const [rows] = await db_1.default.execute(`SELECT * FROM sf2_reports ORDER BY created_at DESC LIMIT 50`);
        res.json(rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
