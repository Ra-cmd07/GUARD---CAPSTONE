    ...
    try {
        ...
        const insertPromises = devices.map(async (device) => {
            ...
            try {
                ...
                const [students] = await db_1.default.execute(`SELECT id FROM students 
           WHERE REPLACE(REPLACE(LOWER(mac_address), ':', ''), '-', '') = ?`, [normalizedMac]);
                ...
                await db_1.default.execute(`INSERT INTO BLEPROXY (room_name, mac_address, distance, rssi, timestamp)
           VALUES (?, ?, ?, ?, ?)`, [roomName, macAddress, distance, rssi, timestamp]);
                insertedCount.value++;
            }
            catch (err) {
...
async function detectBleToken(req, res) {
    try {
        const { student_id, student_name, mac, rssi, distance, kiosk_id, gate_name } = req.body;
        ...
        const [students] = await db_1.default.execute(`SELECT id, name, grade, section FROM students WHERE id = ?`, [student_id]);
        if (students.length === 0) {
       ...
       WHERE student_id = ? 
       AND status = 'pending' 
       AND detected_at > DATE_SUB(NOW(), INTERVAL 30 SECOND)`, [student_id]);
       ...
       VALUES (?, ?, ?, ?, ?, ?, 'pending')`, [student_id, student_name, mac, rssi || null, kiosk_id || 1, gate_name || 'Main Gate']);
