...
exports.getActiveClassForSection = getActiveClassForSection;
exports.getActiveClassesForTeacher = getActiveClassesForTeacher;
exports.getNextClassForSection = getNextClassForSection;
 ...
 */
async function getActiveClassForSection(section_id, current_time, day_of_week) {
    try {
        const [results] = await db_1.default.execute(`SELECT tc.id
       ...
       WHERE tc.section_id = ?
         ...
         AND tc.time_end >= ?
         AND tc.is_active = 1
       LIMIT 1`, [section_id, day_of_week, current_time, current_time]);
 ...
 */
async function getActiveClassesForTeacher(teacher_id, current_time, day_of_week) {
    try {
        const [results] = await db_1.default.execute(`SELECT tc.id
       FROM teacher_classes tc
       WHERE tc.teacher_id = ?
         ...
         AND tc.time_start <= ?
         AND tc.time_end >= ?
         AND tc.is_active = 1`, [teacher_id, day_of_week, current_time, current_time]);
        if (results && results.length > 0) {
 ...
 */
async function getNextClassForSection(section_id, current_time, day_of_week) {
    try {
        const [results] = await db_1.default.execute(`SELECT tc.id, tc.subject, tc.teacher_id, tc.time_start, tc.time_end
       FROM teacher_classes tc
       WHERE tc.section_id = ?
         AND tc.day_of_week = ?
         AND tc.time_start > ?
       ...
       LIMIT 1`, [section_id, day_of_week, current_time]);
        if (results && results.length > 0) {
            return results[0];
        }
        return null;
