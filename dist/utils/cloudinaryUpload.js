 ...
 */
async function uploadToCloudinary(localPath, studentName) {
    try {
        // Convert relative path to absolute
        const absolutePath = localPath.startsWith('/uploads/')
            ? path_1.default.join(__dirname, '..', localPath.replace('/uploads/', 'uploads/'))
            : localPath;
        ...
        const result = await cloudinary_1.v2.uploader.upload(absolutePath, {
            folder: folderPath,
            resource_type: 'image',
            format: 'jpg',
            transformation: [
 ...
 * @returns Cloudinary upload result with CDN URL
 */
async function uploadBase64ToCloudinary(base64Data, studentName, prefix = 'scan') {
    try {
        ...
        // Upload to Cloudinary
        const result = await cloudinary_1.v2.uploader.upload(base64WithPrefix, {
            folder: folderPath,
            resource_type: 'image',
            format: 'jpg',
