...
import type { GuardianForm, StudentForm } from '../types/index.tsx';

const DEFAULT_GUARDIANS: GuardianForm[] = [
...
export default function StudentsPage() {
  ...
  const [guardians,  setGuardians]  = useState<GuardianForm[]>(DEFAULT_GUARDIANS);
  const [loading,    setLoading]    = useState(false);
  const [qrOpen,     setQrOpen]     = useState(false);
  const [registered, setRegistered] = useState<{ student: StudentForm; guardians: GuardianForm[] } | null>(null);
  const [snack, setSnack] = useState<{ open: boolean; msg: string; sev: 'success'|'error' }>
                              ({ open: false, msg: '', sev: 'success' });

  const updateGuardian = (idx: number, field: keyof GuardianForm, val: string) =>
    setGuardians(g => g.map((item, i) => i === idx ? { ...item, [field]: val } : item));
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/students', { ...student, parents_guardians: guardians });
      setRegistered({ student, guardians });
      setQrOpen(true);
              ...
              {guardians.map((g, idx) => (
                <Box key={idx} mb={2}>
                  ...
                  <TextField label={`${g.role} Name`} fullWidth margin="dense"
                    value={g.name} onChange={e => updateGuardian(idx, 'name', e.target.value)} />
                  <TextField label="Contact Number" fullWidth margin="dense"
                    value={g.contact_number}
                    onChange={e => updateGuardian(idx, 'contact_number', e.target.value)} />
                </Box>
              ))}
            </Paper>
          </Grid>
