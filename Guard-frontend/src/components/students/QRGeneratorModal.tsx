...
export default function QRGeneratorModal({ open, onClose, student, guardians }: Props) {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle sx={{ background: '#2d5016', color: '#fff', fontWeight: 700 }}>
      ...
      <DialogContent sx={{ p: 3 }}>
        <Typography variant="body2" color="text.secondary" textAlign="center" mb={3}>
          Each guardian/parent has a unique QR code linked to <b>{student.name}</b> (LRN: {student.lrn})
        </Typography>

        <Grid container spacing={3} justifyContent="center">
          {guardians.filter(g => g.name).map(g => (
            <Grid item xs={12} sm={4} key={g.role}>
      ...
      <DialogActions sx={{ px: 3, pb: 2 }}>
        ...
        <Button variant="contained" color="primary"
          onClick={() => guardians.filter(g => g.name).forEach(g => {
